...
class KDTree():
    ...
    def insert(self, current_data, split_dim):
        ...
        data_sorted = sorted(current_data, key=lambda x: x[split_dim])  # sort by segmentation dimension from smallest to largest

        # the following three sentences of code are essentially post-order traversals of binary trees
        lchild = self.insert(data_sorted[0:mid], self.cal_split_dim(split_dim))  # recursively construct the left subtree
        rchild = self.insert(data_sorted[mid + 1:], self.cal_split_dim(split_dim))  # recursively construct the right subtree
        return Node(lchild, rchild, data_sorted[mid], split_dim)  # join the left and right subtrees from the root node and return
